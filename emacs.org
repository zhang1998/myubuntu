...
C-c / t	在稀疏树中显示 TODO 项。将 buffer 折叠，但是会显示 TODO 项和它们所在的层次的标题。
    以后使用todo的问题，没有正在处理的问题，尽量只加入第一级标题而不是全部
...
<字母 tab   ：模板的使用  [[https://blog.csdn.net/u014801157/article/details/51103223][一篇教程]]
M-x kill-some-buffers   杀死buffer
c-c c-t 切换标记：todo done等
c- 方向键 切换窗口
todo
    将要做的事情缩减为一行
s m r/l  携带子标题进行目录级别变动
删除
    同样是删除，选中删除和c-k 不同，一个会进入测试
    c-k 会进入测试
    delete不会
录制宏：这是一个和vim类似的宏
    C-x (
...
网页另存为或者在网页上做笔记
支持多文件配置
m-x  company-complete-common 另有[[https://blog.csdn.net/meteor1113/article/details/4349281][一篇详细的博客]]
emacs-w3m
| c-c c-s | choose buffer          |
| c-c c-n | next buffer            |
| c-c c-e | new  buffer with blank |
|         |                        |
*** todo时间问题
...
******* 循环和时间段的差异问题
时间段带[已进行天数/全部天数] 但是我如何知道自己今天已经做过了呢 有紧迫感
猜测使用next即可
***** 截止期限 c-c c-d 
